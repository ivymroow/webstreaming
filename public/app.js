...
async function loadEpisodeSources(id,season,episode){
  ...
  let epHTML=''
  if(d.unavailable){
    epHTML='<div class="unreleased-warning" style="padding:16px;background:rgba(239,68,68,0.1);border:1px solid #ef4444;border-radius:var(--radius);color:#fc8181;margin-top:16px;font-size:14px;line-height:1.5"><strong>Unavailable:</strong> '+esc(d.unavailableReason||'This title is currently unavailable.')+'</div>'
    ...
    else{const f=episodes.find(x=>x.season!==0)||episodes[0];selectedSeason=f?.season??1;selectedEpisode=f?.episodes?.[0]?.number||1}
    window._eps=episodes
    const seasonList=[...episodes].sort((a,b)=>a.season===0?-1:(b.season===0?1:a.season-b.season))
    epHTML='<div class="episode-picker"><div class="sources-title">Select Episode</div><div class="episode-controls"><select id="seasonSelect">'+seasonList.map(s=>'<option value="'+s.season+'">'+(s.season===0?'Specials / holiday episodes':'Season '+s.season)+' ('+s.episodes.length+' eps)</option>').join('')+'</select><select id="episodeSelect"></select></div><div id="episodeInfo" class="episode-info"></div><div class="sources-section" style="margin-top:12px"><div class="sources-title">Sources</div><div class="sources-list" id="sl"><div class="loading-screen" style="padding:12px"><div class="spinner"></div></div></div></div></div>'
  }
...
function fillEpisodes(isSeasonChange){
  ...
  es.innerHTML=epData.episodes.map(e=>'<option value="'+e.number+'">'+(selectedSeason===0?'Special ':'')+e.number+'. '+esc(e.name)+(e.airdate?' ('+e.airdate+')':'')+'</option>').join('')
