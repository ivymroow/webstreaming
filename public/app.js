...
async function loadEpisodeSources(id,season,episode){
  ...
  if(state.data?._playHash){const ph=state.data._playHash;state.data._playHash=null;setTimeout(()=>{const b=ph==='auto'?qs('#sl .source-play'):qs('#src-'+ph+' .source-play');if(b)b.click()},100)}
  ...
  panel.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid #2d1e3e;background:rgba(20,13,30,0.7)">
      ...
      <div style="display:flex;gap:6px;align-items:center">
        <button onclick="dlGuiPoll();dlGuiSniffCurrentEmbed()" style="border:1px solid #38254c;background:#1a1126;color:#c4b5d4;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px">Scan</button>
...
async function dlGuiSniffCurrentEmbed(){
  const activeUrl=qs('#playerIframe')?.src||state._embedUrl||''
  if(!activeUrl||state.mode!=='backend')return
...
async function doAuth(){
  ...
  try{
    if(err){err.style.color='var(--text-muted)';err.textContent=authMode==='signup'?'creating account...':'signing in...'}
