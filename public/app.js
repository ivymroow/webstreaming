...
function G(id,items){
  ...
  el.innerHTML=items.map(i=>{const p=img(i.poster),t=title(i),y=year(i),r=rating(i),tp=i.type;(i.progress||0)>0
    ...
    return'<div class="card" data-id="'+i.id+'" data-title="'+esc(title(i))+'" data-poster="'+(i.poster||'')+'" onclick="navigate(\'detail\',{id:\''+i.id+'\',type:\''+(tp||'movie')+'\',title:\''+esc(title(i))+'\',year:\''+(year(i)||'')+'\',poster:\''+(i.poster||'')+'\''+(se?',season:'+i.season+',episode:'+i.episode:'')+'})"><img class="card-poster" src="'+p+'" alt="'+esc(t)+'" loading="lazy" referrerpolicy="no-referrer" onerror="this.src=\'data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 200 300%22><rect fill=%22%231a1a1a%22 width=%22200%22 height=%22300%22/><text x=%22100%22 y=%22160%22 text-anchor=%22middle%22 font-size=%2248%22 fill=%22%23555%22>🎬</text></svg>\'"><span class="card-badge">'+(tp==='tv'?'TV':'Movie')+'</span>'+(r?'<span class="card-rating"><svg viewBox="0 0 24 24" width="10" height="10" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>'+r+'</span>':'')+(i.progress?'<div class="progress-bar"><div class="progress-fill" style="width:'+Math.min(i.progress*100,100)+'%"></div></div>':'')+'<div class="card-info"><div class="card-title">'+esc(t)+'</div><div class="card-meta">'+se+(se&&y?' · ':'')+(y||'')+'</div></div></div>'}).join('')
}
