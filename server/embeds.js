async function getEmbeds(imdbId, tmdbId, season, episode) {
  ...
  if (season && episode) {
    ...
    if (tmdbId) embeds.push({ name: 'VidSrc', url: `https://vidsrc.to/embed/tv/${tmdbId}/${season}/${episode}` });
    ...
    if (tmdbId) embeds.push({ name: 'VidSrc', url: `https://vidsrc.to/embed/movie/${tmdbId}` });
    embeds.push({ name: 'MultiEmbed', url: `https://multiembed.mov/?video_id=${imdbId}` });
